...
CANDIDATE_SHA="cc06ffe37fea4a58a225a25f464b85ad59d5554b"
CANDIDATE_PATCH_SHA256="fdbbab0abc477a946a7690e637c5938c0d99ab142b4cf614a1bbd98d05eee727"
...
readonly CONTROL_SHA CANDIDATE_SHA CANDIDATE_PATCH_SHA256 KERNELSU_SHA
...
verify_source() {
    ...
    local actual_patch patch_sha color_source plane_source python_bin
    ...
    expected_files=$(printf '%s\n' \
        'techpack/display/msm/sde/sde_color_processing.c' \
    ...
    patch_sha=$(sha256_file "$actual_patch")
    require_equal "$CANDIDATE_PATCH_SHA256" "$patch_sha" \
        "candidate PCC patch hash"
    cp "$actual_patch" "$output_dir/candidate-pcc-oneui.patch"
    rm -f "$actual_patch"

    color_source=$(mktemp)
    plane_source=$(mktemp)
    git -C "$repo" show \
        "$CANDIDATE_SHA:techpack/display/msm/sde/sde_color_processing.c" \
        > "$color_source"
    git -C "$repo" show \
        "$CANDIDATE_SHA:techpack/display/msm/sde/sde_plane.c" \
        > "$plane_source"
    python_bin=$(command -v python3 || command -v python) ||
        fail "python interpreter not found"
    "$python_bin" - "$color_source" "$plane_source" <<'PY'
from pathlib import Path
import sys

color = Path(sys.argv[1]).read_text()
plane = Path(sys.argv[2]).read_text()
...
require(color.count("#include <linux/rom_notifier.h>") == 1,
        "rom_notifier include gate failed")
require(color.count("ret = set_feature(hw_dspp, &hw_cfg, sde_crtc);") == 1,
        "DSPP wrapper gate failed")
require(color.count("if (is_aosp) {\n\t\t\t\t\thw_cfg.payload = NULL;") == 1,
        "AOSP DSPP payload gate failed")
require(plane.count("_sde_plane_set_csc_pcc(psde, pstate, crtc);") == 1,
        "PCC-to-CSC call count gate failed")
require(plane.count(
    "if (is_aosp) {\n"
    "\t\t_sde_plane_set_fod_dim_alpha(psde, pstate);\n"
    "\t\t_sde_plane_set_csc_pcc(psde, pstate, crtc);\n"
    "\t}"
) == 1, "AOSP plane PCC policy gate failed")
PY
    rm -f "$color_source" "$plane_source"

    git -C "$repo" show "$CANDIDATE_SHA:include/linux/rom_notifier.h" |
        grep -Fxq 'extern bool is_aosp __read_mostly;'
...

    {
        ...
        printf 'pcc_policy=global-dspp-oneui,per-plane-csc-aosp\n'
    ...
    (
        cd "$output_dir"
        sha256sum SOURCE-VERIFICATION.txt candidate-pcc-oneui.patch \
            > SOURCE-SHA256SUMS
    )
}
