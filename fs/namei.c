...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_should_hide_inode(nd->link_inode)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_should_hide_inode(inode)) {
		return -ENOENT;
	}
#endif
		...
		struct dentry *base, unsigned int flags)
{
	...
	if (dentry) {
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (!IS_ERR(dentry) && susfs_should_hide_inode(dentry->d_inode)) {
			dput(dentry);
			return ERR_PTR(-ENOENT);
		}
#endif
	...
	if (nd->flags & LOOKUP_RCU) {
		...
		*seqp = seq;
		...
		if (unlikely(!dentry))
			return 0;
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (susfs_should_hide_inode(d_backing_inode(dentry))) {
			dput(dentry);
			return -ENOENT;
		}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (susfs_should_hide_inode(nd->inode)) {
			return -ENOENT;
		}
#endif
	}
		    ...
		    struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && susfs_should_hide_inode(path->dentry->d_inode)) {
		path_put(path);
		retval = -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_should_hide_inode(child->d_inode)) {
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (susfs_should_hide_inode(dentry->d_inode)) {
